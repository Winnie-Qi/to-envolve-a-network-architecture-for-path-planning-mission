...
class DefaultGenomeConfig(object):
    ...
    def __init__(self, params):
        ...
        self._params = [ConfigParameter('num_inputs', int),
                        ...
                        ConfigParameter('single_channel_mutate_ratio', float),
                        ConfigParameter('single_channel_mutate_rate', float),
                        ConfigParameter('each_channel_mutate_prob', str),
                        ConfigParameter('parallel_layer', int),
                        ConfigParameter('parallel_nodes_every_layer', str)]
...
class DefaultGenome(object):
    ...
    def __init__(self, key, config):
        ...
        for i in range(len(self.layers), len(self.layers) + config.dense_after_cnn):
            self.layers.append(['fc', []])
        for i in range(len(self.layers), len(self.layers) + config.num_gnn_layer):
            self.layers.append(['gnn', []])
        for i in range(len(self.layers), len(self.layers) + config.dense_after_gnn):
            self.layers.append(['fc', []])

        self.nodes_every_layer = [int(x) for x in config.nodes_every_layer.split(',')]
        ...
        self.parallel_nodes_every_layer = [int(x) for x in config.parallel_nodes_every_layer.split(',')]

        self.cnn_nodes_conv_layer = []
        for i in range(self.num_cnn_layer):
            self.cnn_nodes_conv_layer.append([-1] * self.nodes_every_layer[i])
        ...
        self.direct_conn = {}
        self.each_channel_mutate_prob = [float(x) for x in config.each_channel_mutate_prob.split(',')]

        # Fitness results.
        self.fitness = 0

        self.par_nodes = {}
        self.par_connections = {}
        self.par_layers = {}
        for i in range(config.parallel_layer + 1):
            self.par_layers[i] = []
        self.num_parallel_fc = 1

    def configure_new(self, config, pretrained_fc):
        """Configure a new genome based on the given configuration."""

        # Create node genes for the output pins.
        for node_key in config.output_keys:
            self.nodes[node_key] = self.create_node(config, node_key, len(self.layers)-1, 'fc', '_')
        # Add output layer nodes
        self.layers[-1][1] = list(config.output_keys)
        ...
        for i in range(config.num_cnn_layer):
            for j in range(self.nodes_every_layer[i]):
                ...
                if i == 0:
                    node = self.create_node(config, node_key, i, 'cnn', [3,self.nodes_every_layer[i]])
                else:
                    node = self.create_node(config, node_key, i, 'cnn', self.nodes_every_layer[i-1:i+1])
                self.nodes[node_key] = node
                self.layers[i][1].append(node_key)
        ...
        # Create node genes for the parallel fc nodes
        with open(pretrained_fc, 'rb') as f:
            pretrained_par = pickle.load(f)
        for node_key in config.output_keys:
            self.par_nodes[node_key] = self.create_node(config, node_key, 1, 'fc', '_')
            self.par_nodes[node_key].bias = pretrained_par['fc2.bias'][node_key]
            self.par_layers[1].append(node_key)

        for i in range(self.parallel_nodes_every_layer[0]):
            self.par_nodes[i + config.num_outputs] = self.create_node(config, i + config.num_outputs, 0, 'fc', '_')
            self.par_nodes[i + config.num_outputs].bias = pretrained_par['fc1.bias'][i]
            self.par_layers[0].append(i + config.num_outputs)
            for j in config.output_keys:
                connection = self.create_connection(config, (i + config.num_outputs, j),
                                                    [self.parallel_nodes_every_layer[0] * 2, config.num_outputs * 2], (0, 1))
                connection.weight = pretrained_par['fc2.weight'][j][i]
                self.par_connections[connection.key] = connection
        for index_i, i in enumerate([-1, -2]):
            for index_j, j in enumerate(self.par_layers[0]):
                connection = self.create_connection(config, (i, j),
                                                    [5, len(self.par_layers[0])], (-1, 0))
                connection.weight = pretrained_par['fc2.weight'][index_j][index_i]
                self.par_connections[connection.key] = connection
    ...
    def configure_crossover(self, genome1, genome2):
        ...
        for key, cg1 in iteritems(genome1.par_connections):
            cg2 = genome2.par_connections.get(key)
            if cg2 is None:
                # Excess or disjoint gene: copy from the fittest parent.
                self.par_connections[key] = cg1.copy()
            else:
                # Homologous gene: combine genes from both parents.
                self.par_connections[key] = cg1.crossover(cg2)
        ...
        for key, ng1 in iteritems(genome1.nodes):
            ...
            elif isinstance(ng1, DefaultNodeGeneCNN):
                layer = ng1.layer
                if layer != 0:
                    if genome1.nodes_every_layer[layer - 1] != genome2.nodes_every_layer[layer - 1] or ng1.single_channel != ng2.single_channel:
                        self.nodes[key] = ng1.copy()
                    else:
                        self.nodes[key] = ng1.crossover(ng2)
                elif ng1.single_channel != ng2.single_channel:
                    self.nodes[key] = ng1.copy()
                else:
                    self.nodes[key] = ng1.crossover(ng2)
            else:
        ...
        for key, ng1 in iteritems(genome1.par_nodes):
            ng2 = genome2.par_nodes.get(key)
            assert key not in self.par_nodes
            if ng2 is None:
                # Extra gene: copy from the fittest parent
                self.par_nodes[key] = ng1.copy()
            else:
                # Homologous gene: combine genes from both parents.
                self.par_nodes[key] = ng1.crossover(ng2)
        ...
        self.layers = copy.deepcopy(genome1.layers)
        self.maxpooling_mask = copy.deepcopy(genome1.maxpooling_mask)
        self.nodes_every_layer = copy.deepcopy(genome1.nodes_every_layer)
        self.par_layers = copy.deepcopy(genome1.par_layers)
        ...
        for i in range(len(self.cnn_nodes_conv_layer)):
            for j in range(len(self.cnn_nodes_conv_layer[i])):
                node_key = self.layers[i][1][j]
                if self.nodes[node_key].single_channel == False:
                    continue
                self.cnn_nodes_conv_layer[i][j] = self.nodes[node_key].single_channel
    ...
    def mutate(self, config, gid):
        ...
        if config.single_structural_mutation:
            ...
            # if random() < config.single_channel_mutate_rate:
        ...
        if 1:
            for cg in self.connections.values():
                if len(cg.key) == 3:
                    cg.mutate(config, [self.nodes_every_layer[cg.connect_layer[0]] * self.size_output_cnn, self.nodes_every_layer[cg.connect_layer[1]]])
                else:
                    cg.mutate(config, self.nodes_every_layer[cg.connect_layer[0]:cg.connect_layer[1]+1])

        if 0:
            for cg in self.par_connections.values():
                # cg.mutate(config, [])
                if random() < 0.9:
                    cg.weight = cg.weight + gauss(0.0, 0.02)

        if 1:
            for dg in self.direct_conn.values():
                dg.mutate(config, [self.nodes_every_layer[dg.connect_layer[0]],self.nodes_every_layer[dg.connect_layer[1]]])

            # Mutate node genes (bias, kernels).
            for ng in self.nodes.values():
                ng.mutate(config, [self.nodes_every_layer[ng.layer-1], self.nodes_every_layer[ng.layer]])

        if 0:
            for ng in self.par_nodes.values():
                ng.mutate(config, [])
    ...
    def mutate_add_node(self, config, gid):
        ...
        if in_one_layer:
            layer_num = randint(0, len(self.nodes_every_layer) - 2)

        for i in range(config.node_add_num):
            # Choose the layer to add node (not the last layer)
            if not in_one_layer:
                layer_num = randint(0, len(self.nodes_every_layer)-2)
            node_type = self.layers[layer_num][0]

            # Revise the nodes_every_layers and cnn_nodes_conv_layer list
            self.nodes_every_layer[layer_num] += 1
            if layer_num < self.num_cnn_layer:
                self.cnn_nodes_conv_layer[layer_num].append(-1)

            new_node_id = config.get_new_node_key(self.nodes)
            if node_type == 'cnn':
                if layer_num == 0:
                    ng = self.create_node(config, new_node_id, layer_num, 'cnn',
                                          [config.num_inputs, self.nodes_every_layer[layer_num]])
                else:
                    ng = self.create_node(config, new_node_id, layer_num, 'cnn',
                                          self.nodes_every_layer[layer_num-1:layer_num+1])
            ...
            self.layers[layer_num][1].append(new_node_id)
            ...
            if layer_num > self.num_cnn_layer - 1:
                ...
                if layer_num == len(self.nodes_every_layer)-2: # Add connection to output, if the added node in last layer
                    for output_id in config.output_keys:
                        connections.append((new_node_id, output_id))
                else:
                    for j in list(self.layers[layer_num + 1][1]):
                        connections.append((new_node_id, j))
                for node_id in connections:
                    connection = self.create_connection(config, node_id,
                                                        self.nodes_every_layer[layer_num : layer_num + 2],
                ...
                if layer_num == self.num_cnn_layer: # if the added node in the first fc layer after cnn
                    for i in list(self.layers[layer_num - 1][1]):
            ...
            # if the added node in cnn layer but not the last cnn layer ---- add one layer to each kernel of next cnn layer
                ...
                for node_key in self.layers[layer_num+1][1]:
                    kernel_attribute = next((a for a in self.nodes[node_key]._gene_attributes if a.name == 'kernel'),None)
                    new_kernel_layer = kernel_attribute.add_layer(config, self.nodes_every_layer[layer_num:layer_num+2])
            ...
            # if the added node in the last cnn layer ---- add connections
                ...
                for node_id in connections:
                    connection = self.create_connection(config, node_id,
                                                        [self.nodes_every_layer[layer_num] * self.size_output_cnn,
                                                         self.nodes_every_layer[layer_num + 1]], (layer_num, layer_num + 1))
                    self.connections[connection.key] = connection
    ...
    def mutate_add_direct_conn(self, config, gid):
        ...
        out_layer_list = [x for x in range(len(self.nodes_every_layer) - self.dense_after_gnn, len(self.nodes_every_layer))]
        out_layer = choice(out_layer_list)
        available_in_nodes = self.layers[in_layer][1]
        available_out_nodes = self.layers[out_layer][1]
        for i in range(config.direct_conn_num):
            ...
            connection = self.create_connection(config, (in_node, out_node),
                                                [(self.nodes_every_layer[out_layer - 1] + 1), self.nodes_every_layer[out_layer]],
                                                (in_layer, out_layer))
    ...
    def mutate_delete_node(self, config, gid):
        ...
        if in_one_layer:
            layer_num = randint(0, len(self.layers) - self.dense_after_gnn - 1)
            available_nodes = copy.deepcopy(self.layers[layer_num][1])
        ...
        for i in range(config.node_delete_num):
            ...
            while self.nodes_every_layer[layer_num] <= 1:
                del_key = choice(available_nodes)
                layer_num = self.nodes[del_key].layer
            ...
            if layer_num < self.num_cnn_layer - 1: # deleted node in cnn layers but not in the last cnn layer
                ...
                if layer_num < self.num_cnn_layer: # the last cnn layer
                    self.fitness += (len(self.nodes[del_key].kernel) + 1) * config.parameter_cost
                    del_node_index = self.layers[layer_num][1].index(del_key)
                    if layer_num not in deleted_cnn_nodes:
                        deleted_cnn_nodes[layer_num] = [del_node_index]
                    else:
                        deleted_cnn_nodes[layer_num].append(del_node_index)
        ...
        for layer_num in deleted_cnn_nodes:
            indices_to_remove = sorted(deleted_cnn_nodes[layer_num], reverse=True)
            for j in indices_to_remove:
                if layer_num < self.num_cnn_layer - 1:
                    self.cnn_nodes_conv_layer[layer_num + 1] = [-1 if x == j else x for x in
                                                                self.cnn_nodes_conv_layer[layer_num + 1]]
                    self.cnn_nodes_conv_layer[layer_num + 1] = [x - 1 if x > j else x for x in
                                                                self.cnn_nodes_conv_layer[layer_num + 1]]
                self.cnn_nodes_conv_layer[layer_num].pop(j)

        for del_key, layer_num in deleted_nodes:
            self.layers[layer_num][1].remove(del_key)
    ...
    def mutate_add_layer(self, config, gid):
        ...
        if a < config.add_cnn_layer and self.num_cnn_layer < 5: # add cnn layer
            ...
            if a < config.add_layer_halve:
                node_add_num = int(self.nodes_every_layer[self.num_cnn_layer-1] / 2) # number of added nodes halves the last cnn layer
            elif a < config.add_layer_double:
                node_add_num = self.nodes_every_layer[self.num_cnn_layer - 1] * 2 # number of added nodes doubles the last cnn layer
            else:
                node_add_num = self.nodes_every_layer[self.num_cnn_layer - 1] # number of added nodes duplicates the last cnn layer
            ...
            self.nodes_every_layer.insert(layer_num, node_add_num)
            self.cnn_nodes_conv_layer.append([-1] * node_add_num)
            # Add one to the number of layer in the attributes of every node in subsequent layers
            for _, layer in self.layers[layer_num:]:
                for node_key in layer:
                    try:
                        self.nodes[node_key].layer += 1
                    except:
                        self.nodes[node_key].layer += 1
            self.layers.insert(layer_num, ['cnn', []])
            ...
            for i in range(node_add_num):
                new_node_id = config.get_new_node_key(self.nodes)
                ng = self.create_node(config, new_node_id, layer_num, 'cnn',
                                      [self.nodes_every_layer[layer_num - 1], node_add_num])
                self.nodes[new_node_id] = ng
                self.layers[layer_num][1].append(new_node_id)
                self.fitness -= (len(ng.kernel) + 1) * config.parameter_cost
                for j in self.layers[layer_num + 1][1]:
            ...
            if a < config.add_fc_before_gnn: # add a fc layer before the gnn layer
                ...
                layer_num = len(self.nodes_every_layer) - 1
                self.dense_after_gnn += 1
                a_log = 'after gnn layer'

            a = random()
            if a < config.add_layer_halve:
                node_add_num = int(self.nodes_every_layer[layer_num - 1] / 2)  # number of added nodes halves the previous layer
            elif a < config.add_layer_double:
                node_add_num = self.nodes_every_layer[layer_num - 1] * 2  # number of added nodes doubles the previous layer
            else:
                node_add_num = self.nodes_every_layer[layer_num - 1]  # number of added nodes duplicates the previous layer
            ...
            self.layers.insert(layer_num, ['fc', []])
            ...
            for i in range(node_add_num):
                ...
                for i in list(self.layers[layer_num - 1][1]):
                    ...
                    connection = self.create_connection(config, node_id,
                                                        ...
                                                        (layer_num - 1, layer_num))
                    self.connections[connection.key] = connection
                for i in self.layers[layer_num + 1][1]:
                    node_id = (new_node_id, i)
                    connection = self.create_connection(config, node_id,
                                                        [node_add_num, self.nodes_every_layer[layer_num + 1]],
                                                        (layer_num, layer_num + 1))
                    self.connections[connection.key] = connection
            self.fitness -= node_add_num * config.parameter_cost
            self.fitness -= (self.nodes_every_layer[layer_num - 1] * node_add_num *
                             self.nodes_every_layer[layer_num + 1]) * config.parameter_cost
    ...
    def mutate_conv_single_channel(self, config):
        ...
        if not any(item != -1 for sublist in self.cnn_nodes_conv_layer for item in sublist):
            for index, j in enumerate(self.layers[0][1]):
                if self.cnn_nodes_conv_layer[0].count(-1) == 1:
                    unique_elements = set(self.cnn_nodes_conv_layer[0])
                    if not (0 in unique_elements and 1 in unique_elements and 2 in unique_elements):
                        break
                a = random()
                if a < config.single_channel_mutate_ratio:
                    b = random()
                    l = next((i for i, prob in enumerate(self.each_channel_mutate_prob) if b < prob), None)
                    self.cnn_nodes_conv_layer[0][index] = l
                    node_key = self.layers[0][1][index]
                    self.nodes[node_key].single_channel = l

        # only mutate nodes in the layers that have mutated nodes or the next layer
        else:
            input_record = copy.deepcopy(self.cnn_nodes_conv_layer)
            for i in range(len(self.cnn_nodes_conv_layer) - 1, -1, -1):
                if any(elem != -1 for elem in self.cnn_nodes_conv_layer[i]):
                    to_layer = i
                    break
        #     for i in range(to_layer + 2):
        #         if i == 0:
        #             for j, value in enumerate(self.cnn_nodes_conv_layer[0]):
        #                 a = random()
        #                 if a < config.single_channel_mutate_ratio:
        #                     b = random()
        #                     l = next((i for i, prob in enumerate(self.each_channel_mutate_prob) if b < prob), None)
        #                     if l != self.cnn_nodes_conv_layer[0][j] and self.cnn_nodes_conv_layer[0].count(l) == 1:
        #                         self.cnn_nodes_conv_layer[1] = [-1 if x == j else x
        #                                                             for x in self.cnn_nodes_conv_layer[1]]
        #                     self.cnn_nodes_conv_layer[0][j] = l
        #                 # elif a > 0.9 and value != -1:
        #                 #     self.cnn_nodes_conv_layer[0][j] = -1
        #                 #     self.cnn_nodes_conv_layer[i + 1] = [-1 if x == j else x
        #                 #                                         for x in self.cnn_nodes_conv_layer[i + 1]]
        #         elif i < self.num_cnn_layer:
        #             for j, _ in enumerate(self.cnn_nodes_conv_layer[i]):
        #                 a = random()
        #                 if a < config.single_channel_mutate_ratio:
        #                     available_nodes_previous_layer = [index for index, layer in
        #                                                       enumerate(self.cnn_nodes_conv_layer[i - 1]) if layer != -1]
        #                     if available_nodes_previous_layer:
        #                         l = choice(available_nodes_previous_layer)
        #                         self.cnn_nodes_conv_layer[i][j] = l
        #                         if l != self.cnn_nodes_conv_layer[i][j] and self.cnn_nodes_conv_layer[i].count(
        #                                 l) == 1 and len(self.cnn_nodes_conv_layer) - 1 > i:
        #                             self.cnn_nodes_conv_layer[i + 1] = [-1 if x == j else x
        #                                                             for x in self.cnn_nodes_conv_layer[i + 1]]
        #                 # elif a > 0.9:
        #                 #     self.cnn_nodes_conv_layer[i][j] = -1
        #                 #     if len(self.cnn_nodes_conv_layer) - 1 > i:
        #                 #         self.cnn_nodes_conv_layer[i + 1] = [-1 if x == j else x
        #                 #                                             for x in self.cnn_nodes_conv_layer[i + 1]]

            available_nodes = []
            for j in range(to_layer + 2):
                try:
                    sublist = self.cnn_nodes_conv_layer[j]
                except:
                    break
                for k in range(len(sublist)):
                    if sublist[k] == -1:
                        available_nodes.append([j, k])
            for i in available_nodes:
                ...
                if a < config.single_channel_mutate_ratio:
                    ...
                    if i[0] == 0:
                        ...
                        b = random()
                        l = next((i for i, prob in enumerate(self.each_channel_mutate_prob) if b < prob), None)
                        self.cnn_nodes_conv_layer[0][i[1]] = l
                        input_record[0][i[1]] = l
                        node_key = self.layers[0][1][i[1]]
                        self.nodes[node_key].single_channel = l
                    # if not in the first cnn layer, only convolve the node of the previous layer that is already single-conv
                    else:
                        available_nodes_previous_layer = [index for index, value in
                                                          enumerate(self.cnn_nodes_conv_layer[i[0] - 1]) if value != -1]
                        try:
                            if self.cnn_nodes_conv_layer[i[0]].count(-1) == 1:
                                unique_elements = set(input_record[i[0]])
                                if not (0 in unique_elements and 1 in unique_elements and 2 in unique_elements):
                                    continue
                            l = choice(available_nodes_previous_layer)
                            self.cnn_nodes_conv_layer[i[0]][i[1]] = l
                            input_record[i[0]][i[1]] = input_record[i[0]-1][l]
                            node_key = self.layers[i[0]][1][i[1]]
                            self.nodes[node_key].single_channel = l
                        except:
                            pass

    def mutate_depthwise_conv(self, config):
        available_in_nodes = list(self.layers[0][1])
    ...
    def compute_full_connections_with_layer(self, config, i):
        ...
        if self.layers[i-1][0] == 'cnn' and self.size_output_cnn != 1: # previous layer is cnn
            for node_i in self.layers[i-1][1]:
                for node_j in self.layers[i][1]:
        ...
        for i in range(len(self.layers) - 1):
             for node1 in self.layers[i][1]:
                    for node2 in self.layers[i+1][1]:
    ...
    def connect_full(self, config):
        ...
        """
        fc_layer = [i for i in range(config.num_cnn_layer,len(self.layers))]
        for i in fc_layer:
            for node_id in self.compute_full_connections_with_layer(config, i):
                if len(node_id) == 3:
                    connection = self.create_connection(config, node_id,
                                                        [self.nodes_every_layer[i - 1] * self.size_output_cnn,
                                                         self.nodes_every_layer[i]], (i - 1, i))
                else:
                    connection = self.create_connection(config, node_id, self.nodes_every_layer[i-1:i+1],(i-1,i))
                self.connections[connection.key] = connection
