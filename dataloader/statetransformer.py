...
class AgentState:
    def __init__(self, num_agents):
        ...
        self.map_pad = None
        # self.max_x = config_train.map_w-1
        # self.max_y = config_train.map_h-1
    ...
    def setPosAgents(self, state_allagents):
        ...
        channel_allstate = np.zeros_like(self.map_global, dtype=np.int64) # (20,20)

        for id_agent in range(self.num_agents):
            currentX = int(state_allagents[id_agent][0])
            currentY = int(state_allagents[id_agent][1])
            # print(state_allagents[id_agent][0],state_allagents[id_agent][1])
            # print(currentX,currentY)
            channel_allstate[currentX][currentY] = 1

        channel_allstate_pad = np.pad(channel_allstate, self.FOV_width, self.pad_with, padder=0) # (28,28)
    ...
    def toInputTensor(self, goal_allagents, state_allagents):
        # print(state_allagents)
        channel_allstate_pad = self.setPosAgents(state_allagents) #(28,28)

        input_step = np.zeros((self.num_agents,3,11,11))
        for id_agent in range(self.num_agents): #range(3,6):#

            input_step_currentAgent = np.zeros((3,11,11))
            ...
            input_step_currentAgent[0,:,:] = channel_map
            input_step_currentAgent[1,:,:] = channel_goal
            input_step_currentAgent[2,:,:] = channel_state
            input_step[id_agent,:,:,:] = input_step_currentAgent
    ...
    def toSeqInputTensor(self, goal_allagents, state_AgentsSeq, makespan):
        ...
        for step in range(makespan):
            ...
            for id_agent in range(self.num_agents): #range(3,6):#
                ...
                currentX_global = round(state_allagents[id_agent][0])
                currentY_global = round(state_allagents[id_agent][1])
                goalX_global = round(goal_allagents[id_agent][0])
                goalY_global = round(goal_allagents[id_agent][1])
